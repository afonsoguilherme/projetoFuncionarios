    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            <label v-if="visualizando">{{funcionario.cargo.cargo}}</label>
            <select v-else v-model="funcionario.cargo">
              <option v-for="(cargo, index) in cargos" :key="index" v-bind:value="cargo">
              {{cargo.cargo}}
              </option>
            </select>
          </td>
...
export default {
  ...
  created() {
    this.$http
      .get("http://localhost:3000/funcionarios/" + this.id)
      .then(res => res.json())
  ...
  methods: {
    ...
    salvar(_funcionario){
      ...
      this.$http
      .put(`http://localhost:3000/funcionarios/${_funcionarioEditar.id}`, _funcionarioEditar);
